...
def conforme1(request, pk):
    ...
    if request.method == 'POST':
        ...
        if conforme == 'Non conforme':
            book.is_conforme = True
            book.save()
            return redirect('detail', pk=pk)  
        elif conforme == 'conforme':
            book.is_conforme = False 
            book.save()
...
def dashboard(request):
    ...
    book = Book.objects.all().count()
    user = User.objects.all().count()
    nc = Book.objects.filter(is_conforme=True).count()
    c = Book.objects.filter(is_conforme=False).count()
    cs = Book.objects.filter(status=True).count()
    cn = Book.objects.filter(status=False).count()
    if book> 0:
        non_conforme = (nc / book) * 100
    else:
        non_conforme= 0
    if book> 0:
        conforme= (c / book) * 100
    ...
    if book> 0 :
        completed_percentage = (cs / book) * 100
    else:
        completed_percentage = 0
    if book> 0:
        incompleted_percentage = (cn / book) * 100
    else:
    ...
    context = {
        ...
        'conforme': conforme ,
        'nc': nc,
        'incompleted_percentage': incompleted_percentage }
